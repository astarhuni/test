...
function Wt({
  ...
  return (
    (window.fetch = async function (g, C) {
      ...
      if (Z && (Z.includes("/api/Lottery/GetBalance") || Z.includes("/api/webapi/GetUserInfo"))) {
        B.clone()
          .json()
          .then((R) => {
            let bal = Q(Z, R);
    ...
    (XMLHttpRequest.prototype.send = ((g) =>
      function (C) {
        ...
        this.addEventListener("load", function () {
          ...
          try {
            if (this._url && (this._url.includes("GetUserInfo") || this._url.includes("GetBalance"))) {
...
function Ee(n) {
  ...
  function c() {
    return location.origin + "/#/register?invitationCode=" + (sessionStorage.getItem("invitecode") || "");
  }
  window.addEventListener("wg-open-bonus", () => {
    ((t.style.display = "flex"), (o.textContent = c()));
  });
