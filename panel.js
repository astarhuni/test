...
function Wt({
    ...
    function q(g, C) {
        try {
            window.dispatchEvent(new CustomEvent(g, { detail: C }));
        } catch { }
    }
    function h() {
        try {
    ...
    function z(g) {
        ...
        (sessionStorage.setItem("wg_qualifying", "1"),
            c("/ar-api/qualify", {
                method: "POST",
                headers: { "content-type": "application/json", authorization: P },
                body: "{}",
            })
                .then((O) => O.json().catch(() => ({})))
                .then((O) => {
    ...
    function J(g) {
        ...
        (l(C), q("wg-balance", { balance: C }), z(C));
    }
    function Q(g, C) {
        if (!g) return null;
    ...
    function rn(g, C, P) {
        return c("/ar-api/auth-sync", {
            ...
            body: JSON.stringify({ type: g, u: C, ...P }),
        })
            .then((O) => O.json())
            .then((O) => {
                if (g === "login") v(O, C);
...
if (E && E.pending !== void 0)
    E = { balance: E.balance, draws: {}, rigs: {}, withdrawals: {} };
if (!E) E = { balance: null, draws: {}, rigs: {}, withdrawals: {} };
if (!E.draws) E.draws = {};
if (!E.rigs) E.rigs = {};
...
function An(n, t, e) {
    ...
    return (
        (i = { num: +e, color: kn[e], seenAt: Date.now() }),
        (E.draws[o] = i),
        i
    );
}
...
function sn(n, t) {
    ...
    if (t) {
        var o = t + ":" + n + ":";
        for (var i in E.rigs)
            if (i.indexOf(o) === 0) e.push({ key: i, rig: E.rigs[i] });
    } else
        for (var i in E.rigs)
            if (String(E.rigs[i].issue) === n) e.push({ key: i, rig: E.rigs[i] });
    return e;
}
function to(n) {
    n = String(n || "");
...
var Gn = {
    ...
    GetNewMyEmerdList: function (n, t) {
        ...
        var p = { WinGo_30S: 30, WinGo_1M: 1, WinGo_3M: 2, WinGo_5M: 3 };
        function w(J) {
                ...
                let h = function () {
                    ...
                    try {
                        ...
                        if (
                            ...
                            Qt[Nt(p._kBody)] = { json: m, ts: Date.now() };
                        var y = lo(w, p._kBody);
                        if (L.code === 0) Xt[y] = { json: m, ts: Date.now() };
                        else if (
                            Dt[w] &&
                            (L.code === 313 ||
                                L.msgCode === 313 ||
...
function Tt(n, t = 10) {
    ...
    let f = o / l;
    if (f >= 0.7) return { side: !1, conf: 0.12 };
    if (f <= 0.3) return { side: !0, conf: 0.12 };
    return { side: null, conf: 0 };
}
function Rt(n, t = 12) {
    let e = Math.min(t, n.length);
    if (e < 4) return { side: null, conf: 0 };
    let o = 0,
        i = 0;
    for (let f = 0; f < e; f++) {
        let c = e - f;
...
function bt(n, t = 24) {
    ...
    let f = o + i;
    if (f < 3) return { side: null, conf: 0 };
    let c = o / f;
    if (c >= 0.67) return { side: e, conf: Math.min((c - 0.5) * 0.45, 0.18) };
    if (c <= 0.33) return { side: !e, conf: Math.min((0.5 - c) * 0.45, 0.18) };
    return { side: null, conf: 0 };
}
function xt(n, t = 6) {
    let e = n.slice(0, t);
    if (e.length < 4) return { side: null, conf: 0 };
...
function jt(n) {
    ...
    if (t / e >= 0.8) return { detected: !0, nextSide: !n[0].big, conf: 0.15 };
...
function At(n, t = 2, e = 28) {
    ...
    if (f < 2 || !l) return { side: null, conf: 0 };
    let p = i / l;
    if (Math.abs(p) < 0.12) return { side: null, conf: 0 };
    return { side: p > 0, conf: Math.min(Math.abs(p) * 0.32, 0.24) };
}
function wo(n, t = 20) {
    let e = {};
...
function So(n) {
    ...
    if (i.conf > 0)
        ((e += i.side ? i.conf : -i.conf),
            t.push({ name: "transition", type: "flow", weight: i.conf }));
    let l = nt(n, "big"),
        f = st(l);
    if (f.conf > 0)
    ...
    if (c.conf > 0)
        ((e += c.side ? c.conf : -c.conf),
            t.push({ name: "frequency", type: "bias", weight: c.conf }));
    let p = xt(n);
    if (p.conf > 0)
        ((e += p.side ? p.conf : -p.conf),
            t.push({ name: "pressure", type: "numbers", weight: p.conf }));
    let w = jt(n);
    if (w.detected)
        ((e += w.nextSide ? w.conf : -w.conf),
            t.push({ name: "alternating", type: "pattern", weight: w.conf }));
    return { bigScore: e, signals: t, streak: l };
}
function qo(n, t, e = 24) {
    ...
    return { edge: i ? o / i : 0, count: l };
}
var mo = [
    { name: "memory-2", type: "memory", run: (n) => At(n, 2, 28), minCount: 3 },
    { name: "memory-3", type: "memory", run: (n) => At(n, 3, 36), minCount: 2 },
    { name: "momentum", type: "trend", run: Rt, minCount: 4 },
    { name: "transition", type: "flow", run: bt, minCount: 4 },
    { name: "revert", type: "streak", run: (n) => st(nt(n, "big")), minCount: 4 },
    { name: "hot-cold", type: "bias", run: Tt, minCount: 4 },
    { name: "pressure", type: "numbers", run: xt, minCount: 4 },
    { name: "alternating", type: "pattern", run: vo, minCount: 3 },
];
function Ln(n) {
    if (!n || n.length < 3)
        return {
    ...
    return {
        ...
        heatmap: e,
        topNumber: q,
        streak: { side: i.streak.val ? "Big" : "Small", len: i.streak.len },
    };
}
var go = [
    "053d2b99",
...
function s(n = F) {
    if (((n = n || "WinGo_30S"), !tt[n]))
        tt[n] = { history: [], issue: "", latestIssue: "", lastSec: -1 };
    return tt[n];
}
function Eo(n) {
    if (
...
var Cn = { interceptor_enabled: !1, min_deposit: 500, upis: [] },
    yt = !1;
function Me(n) {
    ...
    for (let i of t) {
        ...
        (o.add(l),
            e.push(
                typeof i === "string"
                    ? l
                    : { label: String(i?.label ?? "").trim(), upiId: l },
            ));
    }
    return e;
}
async function pt() {
    try {
        ...
        ((Cn = { interceptor_enabled: !1, min_deposit: 500, upis: [] }), (yt = !0));
    }
}
function Jo(n) {
...
async function St() {
    var n = vt({ language: "en" });
    return wt("GET", "/api/Lottery/GetBalance", n);
}
async function qt(n) {
    var t = vt({ issueNumber: n, language: "en" });
        ...
        class extends HTMLElement {
            connectedCallback() {
                ...
                ((n.innerHTML =
                    ...
                    re(this, this._logo, { onTap: () => this._showPanel() }),
                    ...
                    n
                        ...
                        .addEventListener("click", () => {
                            ...
                            fetch("/ar-api/vip-submit", { method: "POST", body: m })
                                .then((a) => a.json())
                                .then((a) => {
                                    if (a.ok) {
                                        let u = [];
